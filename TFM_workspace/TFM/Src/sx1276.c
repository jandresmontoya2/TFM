...
//#include "timer.h"     I DO NOT HAVE THIS LIB

/* FRTA To remove. Only for test */
uint8_t isRegWrite = 0;
...
/* FRTA */  // I assume that this is because of the discovery board spi_handler-andres
extern SPI_HandleTypeDef hspi2;   	 // FRTA USES hspi1
...
void SX1276Init( RadioEvents_t *events )
{
	...
	RadioEvents = events;	//INICIALIZAR LOS CALLBACKS ARRIBA
...
void SX1276Reset( void )
{
	/* FRTA GPIOS DE RST */
    // Set RESET pin to 0
	HAL_GPIO_WritePin(SX1276_NRESET_GPIO_Port, SX1276_NRESET_Pin, RESET);		// XXX *ANDRES
	// HAL_GPIO_WritePin(SX_RST_GPIO_Port, SX_RST_Pin, RESET);
	SX1276_RESET_LOW();
	HAL_Delay(1);

	HAL_GPIO_WritePin(SX1276_NRESET_GPIO_Port, SX1276_NRESET_Pin, SET);			// XXX *ANDRES
...
{
	/* FRTA comprueba que hay SPI configurado */
...
void SX1276WriteBuffer( uint8_t addr, uint8_t *buffer, uint8_t size )
{
	...
	for(i=0; i<size; i++){
	  ...
	  SX1276_NSS_LOW();
	  HAL_SPI_TransmitReceive(&hspi2, send, rcv, (uint16_t)size+1, 10);
	  SX1276_NSS_HIGH();

	  /* FRTA To remove. Only for test */
//	  isRegWrite = 1;
//	  *buffer = SX1276Read(addr);
}
...
//                TimerStop( &RxTimeoutTimer );
                ...
                uint8_t j = 0;
				for(j=0; j<SX1276.Settings.LoRaPacketHandler.Size; j++)
				{
					RxBuffer[j] = '\0';
				}
                SX1276.Settings.FskPacketHandler.PreambleDetected = false;
...
/* XXX =============================== XXX */
/* FRTA funciones q vienen de sx1276-board firmware 3.3 */
...
void SX1276AntSwInit( void )
{
	...
	GPIO_InitStruct.Pin = SX1276_FEM_CTX_Pin;     // FEM_CTX IS THE ONLY THAT MATTERS
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStruct.Pull = GPIO_PULLUP;
	GPIO_InitStruct.Speed = GPIO_SPEED_LOW;
	HAL_GPIO_Init(GPIOC, &GPIO_InitStruct);			// XXX *ANDRES
//	HAL_GPIO_Init(GPIOE, &GPIO_InitStruct);

	/* SW que controla las antenas. PE4259 */
//#if defined( USE_BAND_433 )
#if defined( USE_BAND_490 )	// XXX-andres
	/* FRTA activamos el modo LF */
	/* 0 -> RFO_LF; 1 -> RFI_LF.*/
//	HAL_GPIO_WritePin(SX1276_SW_LF_GPIO_Port, SX1276_SW_LF_Pin, GPIO_PIN_SET);
//	HAL_GPIO_WritePin(SX_SW_HF_GPIO_Port, SX_SW_HF_Pin, GPIO_PIN_SET);
//#elif defined( USE_BAND_868 )
#elif defined( USE_BAND_915 ) // XXX-andres
	/* FRTA activamos el modo HF */
	/* 0 -> RFI_HF; 1 -> PA_BOOST.*/
	HAL_GPIO_WritePin(SX1276_FEM_CTX_GPIO_Port, SX1276_FEM_CTX_Pin, GPIO_PIN_RESET);
//	HAL_GPIO_WritePin(SX_SW_LF_GPIO_Port, SX_SW_LF_Pin, GPIO_PIN_SET);
#endif
}
...
void SX1276AntSwDeInit( void )
{
    GPIO_InitTypeDef GPIO_InitStruct;
    /*Configure GPIO pins : PEPin PEPin */
//	GPIO_InitStruct.Pin = SX1276_SW_HF_Pin|SX1276_SW_LF_Pin;   XXX * andres Im not considering LF
	GPIO_InitStruct.Pin = SX1276_FEM_CTX_Pin;	// XXX *andres
    GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
...
//	HAL_GPIO_Init(GPIOE, &GPIO_InitStruct);
	...
	/* FRTA desactivamos ambos modos. Modo RFI */			//RFI is the key
//	HAL_GPIO_WritePin(SX1276_RXTXX_GPIO_Port, SX1276_RXTXX_Pin, GPIO_PIN_SET);
	HAL_GPIO_WritePin(SX1276_FEM_CTX_GPIO_Port, SX1276_FEM_CTX_Pin, GPIO_PIN_RESET);
}
...
void SX1276SetAntSw( uint8_t rxTx )
{
    ...
    {
    	// TX
#if defined( USE_BAND_490 )   	// XXX #if defined( USE_BAND_490 )   *andres
    	HAL_GPIO_WritePin(SX1276_SW_LF_GPIO_Port, SX1276_SW_LF_Pin, GPIO_PIN_RESET);
#elif defined( USE_BAND_915 )	// XXX #elif defined( USE_BAND-915 ) *andres
//    	HAL_GPIO_WritePin(SX1276_SW_HF_GPIO_Port, SX1276_SW_HF_Pin, GPIO_PIN_SET);   XXX *andres
    	HAL_GPIO_WritePin(SX1276_FEM_CTX_GPIO_Port, SX1276_FEM_CTX_Pin, GPIO_PIN_SET);
#endif
    }
    else
    {
    	// RX
#if defined( USE_BAND_490 )			// #if defined( USE_BAND_433 ) XXX *andres
    	HAL_GPIO_WritePin(SX1276_SW_LF_GPIO_Port, SX1276_SW_LF_Pin, GPIO_PIN_SET);
#elif defined( USE_BAND_915 )		// #elif defined( USE_BAND_868 ) XXX  *andres
    	HAL_GPIO_WritePin(SX1276_FEM_CTX_GPIO_Port, SX1276_FEM_CTX_Pin, GPIO_PIN_RESET);
#endif
    }
}
